...
def selectPoints(image, numPoints):
    # to store the random vertices
    randomVertices = []
    # generate numPoints random points
    for i in range(numPoints):
        # select a random integer from 0 to length of image
        randX = np.random.randint(0, image.shape[1])
        # select a random integer from o to breadth of the image
        randY = np.random.randint(0, image.shape[0])
        # only add the vertex if it is valid
        if (image[randY][randX][0] > 205) and (image[randY][randX][1] > 205) and (image[randY][randX][2] > 205):
            randomVertices.append([randX, randY])
        # return the random vertices
...
def saveSol(graph, parents, vertices, track):
    ...
    with open('prm_path.pkl', 'wb') as f:
...
def plotRandomPoints(image, vertices):
    plt.imshow(image)
...
def main():
    ...
    # get the vertices randomly
    vertices = selectPoints(demoTrack, 501)
    # arrange them into an array
    vertices = np.vstack([initialPts, vertices])
    plotRandomPoints(demoTrack, vertices)
    # make the graph object
    prmGraph = PRM(vertices=vertices, track=demoTrack)
    # create the graph
    prmGraph.create_graph()
    # plot the graph
    prmGraph.plot_graph()
    # find the best path, save it, and plot it
    dijkstra(prmGraph.adj_matrix, 0, vertices, demoTrack)
